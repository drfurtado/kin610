...
The assumptions of parametric tests are met when the data is approximately normal and when the sample size is large enough. Therefore, it is recommended to check for normality of the data using normality test such as the Shapiro-Wilk test and check for outliers using box plots and histograms.
...
4.  Random sampling: The data is randomly sampled from the population, so that the sample is representative of the population.
...
Dependent variable: Motor skill learning
Independent variables: Practice schedule (random vs. blocked) and feedback (verbal vs. visual)
...
4.  The data is free from outliers
...
1.  The data is ordinal or continuous, not categorical.
...
3.  The data is at least ordinal, it doesn't assume normality.

4.  The data is independent and randomly selected from the population.
...
It is important to note that the Wilcoxon signed-rank test is a non-parametric test that is used when the data is not normally distributed or the sample size is small. This test is used to compare two related samples or matched-pairs data.
...
The Mann-Whitney U test is particularly useful when the data is not normally distributed, when the sample size is small, or when the data has outliers. It is also used when the data is ordinal or non-normally distributed, and when the variances of the two groups are not equal.
...
5.  No normal distribution: The data does not need to follow a normal distribution, which makes the test a good option when dealing with non-normal data.
...
It is important to note that the Mann-Whitney U test is a non-parametric test that is used to compare two independent samples when the data is not normally distributed. This test is also useful to compare two groups when the sample size is small.
...
Spearman's rho should be used when your data is ordinal or interval and non-normally distributed. Or when you suspect that the relationship between the two variables is non-linear.
